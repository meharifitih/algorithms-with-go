...
func FindTwoThatSum(numbers []int, sum int) (int, int) {
	...
	for i, val := range numbers {
		...
		// using the first iteration iterate over the numbers and find the other number
		for j, val2 := range numbers {
			// check if the both numbers are the same index
			if i == j {
				continue
			}

			// check if the if the sum is the correct value
			if val2+val == sum {
				return i, j
			}
